...
class GraphOptimizer:
    ...
    def _operator_fusion_pass(self):
        ...
        for name in node_names:
            ...
            # Rule 1: Skip branching nodes — they cannot be fused here.
            # Unfused BN/Activation survivors are handled by _branching_bn_activation_pass.
            if len(node.outputs) != 1:
                continue
    ...
    def _branching_bn_activation_pass(self):
        """
        Second fusion pass for BN and Activation nodes that survive the first pass
        because their parent was a branching node (multiple outputs — e.g. ResNet
        skip connections).

        Strategy: instead of fusing into the parent (which has multiple consumers),
        fold the BN math into the BN node itself and re-tag it as a 'ScaleBias' op,
        and re-tag standalone Activation nodes with their activation type in config.
        The compiler then handles both with dedicated generators rather than
        silently treating them as identity.
        """
        folded_count = 0
        retagged_count = 0
        node_names = [n.name for n in self.execution_order]
        ...
        for name in node_names:
            ...
            # --- BN that could not be fused into its parent ---
            if node.op_type == "BatchNormalization" and len(node.weights) == 4:
                gamma, beta, mean, var = node.weights
                epsilon = node.config.get('epsilon', 1e-3)

                # Pre-compute scale and shift so the compiler emits:
                #   out[i] = scale[i] * in[i] + shift[i]
                scale = gamma / np.sqrt(var + epsilon)
                shift = beta - mean * scale

                node.op_type = "ScaleBias"
                node.weights = [scale, shift]   # two 1-D arrays, one per channel
                folded_count += 1

            # --- Standalone Activation / ReLU that could not be fused ---
            elif node.op_type in ["Activation", "ReLU", "LeakyReLU", "Softmax"]:
                if node.op_type == "ReLU":
                    act = "relu6" if node.config.get('max_value') == 6.0 else "relu"
                elif node.op_type == "Softmax":
                    act = "softmax"
                else:
                    act = node.config.get('activation', 'linear')

                # Re-tag as a generic Activation node with a resolved type
                node.op_type = "StandaloneActivation"
                node.config['activation'] = act
                retagged_count += 1

        print(f"  > [Branching BN/Act Pass] Folded {folded_count} BN nodes, "
              f"retagged {retagged_count} activation nodes.")
    ...
    def optimize(self):
        ...
        self._branching_bn_activation_pass()   # handles survivors from branching nodes
        self._dce_pass()
