...
def load_cifar100_data():
    ...
    x_train = x_train.astype('float32') / 255.0
    x_test = x_test.astype('float32') / 255.0
    # No flattening: ResNet expects (N, 32, 32, 3)
...
def main():
    ...
    # Removed cifar10 from choices
    parser.add_argument('--dataset', type=str, default='mnist', choices=['mnist', 'cifar100'])
    parser.add_argument('--model_type', type=str, default='mlp', choices=['mlp', 'resnet'])
    parser.add_argument('--model_path', type=str, default=None)
    parser.add_argument('--distill', type=str, default='none', help="ours, external_hint, etc.")
    ...
    if args.model_path is None:
        if args.model_type == 'mlp':
            args.model_path = "external/mnist_mlp_model.h5"
        else:
            print("!!! Error: --model_path is required for ResNet tests (e.g., resnet110_optimized.h5)")
            exit(1)
    ...
    # --- C. PHASE 1: DISTILLATION (Creation of Optimized Base) ---
    if args.distill != 'none':
        ...
        if args.model_type == 'mlp':
            ...
            # For ResNet, use the teacher's architecture (Self-Distillation)
            print("  > Cloning Teacher architecture as Student for Self-Distillation/Finetuning...")
    ...
    # --- D. PHASE 2-5: OPTIMIZATION PIPELINE (Strategy Manager) ---
    compile_config = {
    ...
    if args.prune or args.quantize:
        output_filename = f"model_{args.dataset}_{args.distill}.cpp"
        ...
        output_filename = f"model_{args.dataset}_baseline.cpp"
        compiler = Compiler(opt_graph)
        compiler.compile(output_filename)
